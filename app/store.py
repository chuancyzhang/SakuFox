...
class DatabaseStore:
    ...
    def _run_migrations(self) -> None:
        ...
        try:
            ...
            with self.engine.begin() as conn:
                ...
                conn.execute(text("ALTER TABLE sandboxes ADD COLUMN knowledge_bases JSON"))
        ...
        try:
            ...
            with self.engine.begin() as conn:
                ...
                conn.execute(text("ALTER TABLE skills ADD COLUMN updated_at VARCHAR(50)"))
    ...
    def create_knowledge_base(self, data: dict) -> str:
    ...
    def get_knowledge_base(self, kb_id: str) -> dict | None:
        with self.SessionFactory() as sess:
            kb = sess.get(DBKnowledgeBase, kb_id)
            if kb:
                return {
                    "id": kb.id,
                    "name": kb.name,
                    ...
                    "api_url": kb.api_url,
                    "api_method": kb.api_method,
                    "api_headers": kb.api_headers or {},
                    "api_params": kb.api_params or {},
                    "api_json_path": kb.api_json_path,
    ...
    def list_knowledge_bases(self) -> list[dict]:
        with self.SessionFactory() as sess:
            kbs = sess.execute(select(DBKnowledgeBase).order_by(DBKnowledgeBase.created_at.desc())).scalars().all()
            return [self.get_knowledge_base(kb.id) for kb in kbs]
